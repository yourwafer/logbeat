...
public class XlsxReader implements ResourceReader {
    ...
    public <E> Iterator<E> read(InputStream input, Class<E> clz) {
        try {
            ...
            throw new RuntimeException("读取EXCEL文件异常", e);
        }
    }
            ...
            try {
                if (mapped) {
                    ...
                    if (keyType == String.class) {
                        ...
                        key = JSON.parseObject(keyName, keyType);
                    }
                    Object value;
                    if (valueType == String.class) {
                        value = content;
                    } else {
                        value = JSON.parseObject(content, valueType);
                    }
        ...
        private void parseRow() {
            ...
            if (idGetter.getValue(instance) == null) {
                logger.error("数值表[{}]的[{}]分页第[{}]行的配置内容[{}]错误 - 主键列NULL", new Object[]{clz.getSimpleName(),
                        sheetName, currentRow, JSON.toJSON(instance)});
            }
